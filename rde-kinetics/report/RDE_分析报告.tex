...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{Fig0_Polarization_1组.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{Fig1_Levich.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{Fig2_Concentration.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{Fig3_KL_1组.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{Fig4_Tafel_1组.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{Fig5_HalfWave_1组.pdf}
...
\begin{enumerate}[leftmargin=1.5cm]
  \item \textbf{Butler-Volmer 反反应贡献量化}：对单电子转移（$n=1$），
  阳极 Tafel 方程 $\eta = \frac{RT}{\alpha F}\ln\frac{i}{i_0}$ 在低 $\eta$ 区
  因反反应贡献不完全成立。B-V 方程取 $\eta=80$\,mV、$\alpha=0.5$ 时反反应贡献
  $\approx \exp(-\alpha F\eta/RT) \approx 17\%$，使表观 Tafel 斜率从理论
  118\,mV/dec 降至 $\sim 90$\,mV/dec，与实测 60--82\,mV/dec 定性一致。
  残余差异的精确归因需 $\eta > 200$\,mV 或采用 Nicholson 方法\cite{nicholson1965}。
  \item \textbf{阳极平台不完整}：$D_{\text{R}}$ 系统偏低 34\%。误差传播分析表明，
  $D_{\text{O}}$ 的 14\% 偏差可由称量误差（$c^* \pm 5\%$，贡献 $\sim 7.5\%$）和
  电极面积误差（$A \pm 3\%$，贡献 $\sim 4.5\%$）联合解释；而 $D_{\text{R}}$ 的
  34\% 偏差远超不确定性传播范围，确认其主要来源为阳极极限电流平台在 0.5 V 处未完全
  达到稳态。基线扣除后 D 值变化 $< 1\%$，排除背景电流为主要贡献。
