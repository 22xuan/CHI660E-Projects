...
\textbf{分析工具：} Python (NumPy, SciPy, Pandas, Matplotlib) \quad \textbf{辅助：} OriginPro（Windows） \\
...
\begin{enumerate}[leftmargin=1.5cm]
  ...
  \item \textbf{图表输出}：Matplotlib 生成 PDF 矢量图，OriginPro 生成 EMF 矢量图（Windows）
\end{enumerate}
...
6. OriginPro（可选） & \texttt{origin\_plot\_orr.py} & \texttt{origin\_input/} & EMF 图 \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}[leftmargin=1.5cm]
  ...
  \item \textbf{图形双轨输出}：Matplotlib 生成 PDF 矢量图（默认），OriginPro 生成 EMF 矢量图（Windows 辅助）
