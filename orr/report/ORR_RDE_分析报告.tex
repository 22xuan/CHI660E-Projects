...
\textbf{分析工具：} Python (NumPy, SciPy, Pandas, Matplotlib) + OriginPro \\
...
\begin{enumerate}[leftmargin=1.5cm]
  ...
  \item \textbf{图表输出}：Matplotlib 生成 PNG 预览图，OriginPro 生成出版级矢量图
\end{enumerate}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{Fig2_Group_Comparison_1600r.jpg}
...
\centering
\includegraphics[width=\textwidth]{Fig3_KL_2组.jpg}
...
\begin{minipage}{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{Fig3_KL_1组.jpg}
...
这表明 ORR 的第一步电子转移（$\text{O}_2 + e^- \rightarrow \text{O}_2^{-\cdot}$）
可能是速率决定步骤（RDS）\cite{spendelow2007}。两种催化剂的 Tafel 斜率接近，
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{Fig4_Tafel_1组.jpg}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{Fig5_Key_Parameters_Bar.jpg}
...
5. 图表 & \texttt{generate\_figures.py} & \texttt{all\_data.csv} & PNG 图 \\
6. OriginPro & \texttt{origin\_plot\_orr.py} & \texttt{origin\_input/} & 出版级图 \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}[leftmargin=1.5cm]
  ...
  \item \textbf{图形双轨输出}：Matplotlib（PNG）+ OriginPro（矢量图），两套工具互补
